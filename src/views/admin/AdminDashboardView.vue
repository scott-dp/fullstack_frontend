...
import { dashboardApi, type DashboardData } from '@/api/operations/dashboard.ts'
    ...
    <template v-else>
      ...
      <div class="stats-grid">
        <div class="stat-card card">
          ...
          <span class="stat-desc">{{ t('In organization') }}</span>
        </div>
        <div class="stat-card card">
          <span class="stat-label">{{ t('Admins') }}</span>
          <span class="stat-value info">{{ countByRole('ROLE_ADMIN') }}</span>
          <span class="stat-desc">{{ t('System administrators') }}</span>
        </div>
        <div class="stat-card card">
          <span class="stat-label">{{ t('Managers') }}</span>
          <span class="stat-value info">{{ countByRole('ROLE_MANAGER') }}</span>
          <span class="stat-desc">{{ t('Compliance managers') }}</span>
        </div>
        <div class="stat-card card">
          <span class="stat-label">{{ t('Staff') }}</span>
          <span class="stat-value">{{ countByRole('ROLE_STAFF') }}</span>
          <span class="stat-desc">{{ t('Operational staff') }}</span>
        </div>
      </div>

      <template v-if="stats">
        <h2>{{ t('Compliance Overview') }}</h2>
        <div class="stats-grid">
          <div class="stat-card card">
            <span class="stat-label">{{ t('Active Templates') }}</span>
            <span class="stat-value">{{ stats.totalChecklistTemplates }}</span>
            <span class="stat-desc">{{ t('Checklist templates') }}</span>
          </div>
          <div class="stat-card card">
      ...
      <div class="admin-grid">
        <router-link to="/app/admin/users" class="action-card card">
          ...
          <div>
            ...
            <p class="text-muted text-sm">{{ t('View and manage user accounts and roles') }}</p>
          </div>
        </router-link>
        <router-link to="/app/checklists" class="action-card card">
          ...
          <div>
            <h3>{{ t('Checklist Templates') }}</h3>
            <p class="text-muted text-sm">{{ t('Create and manage compliance checklists') }}</p>
          </div>
        </router-link>
        <router-link to="/app/deviations" class="action-card card">
          ...
          <div>
            <h3>{{ t('Deviations') }}</h3>
            <p class="text-muted text-sm">{{ t('Review and assign open deviations') }}</p>
          </div>
        </router-link>
        <router-link to="/app/checklists/history" class="action-card card">
          ...
          <div>
            ...
            <p class="text-muted text-sm">{{ t('View checklist completion history') }}</p>
          </div>
        </router-link>
      </div>
    </template>
